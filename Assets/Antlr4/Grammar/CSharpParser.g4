...
argument: (identifier ':')? refout = (REF | OUT)? (VAR | type)? expression;
...
isType: base_type (rank_specifier | '*')* '?'?;
...
simple_embedded_statement:
	...
	| FOREACH OPEN_PARENS local_variable_type identifier IN expression CLOSE_PARENS
...
conversion_operator_declarator: (IMPLICIT | EXPLICIT) OPERATOR type OPEN_PARENS arg_declaration
		CLOSE_PARENS;
...
struct_member_declaration:
	attributes? all_member_modifiers? (
		common_member_declaration
		| FIXED type fixed_size_buffer_declarator+ ';'
	);
...
operator_declaration: // lamdas form C# 6
	OPERATOR overloadable_operator OPEN_PARENS arg_declaration (
		',' arg_declaration
	)? CLOSE_PARENS (body | right_arrow expression ';');

arg_declaration: type identifier ('=' expression)?;
